...
describe('VSO Client Tests Preview.1', function () {
    this.timeout(20000);
    var client,
        fields = [
    ...
    describe('Project tests', function () {
        ...
        it('should return a list of queries', function (done) {
            // We assume an agile team project, so we will need to specifically use the
            // TFS INTEGRATION project
            client.getQueries("TFS INTEGRATION", function (err, queries) {
                if (err) {
                    console.log(err);
                    console.log(queries);
                }
        ...
        it('should return a list of work items from saved query', function (done) {
            ...
            client.getWorkItemIdsByQuery(myBugsQuery.id, "TFS INTEGRATION", function (err, ids) {
                if (err) {
                    //console.log(err);
                    console.log(ids);
                }
        ...
        it('should return a query', function (done) {
            should.exist(myBugsQuery);
            client.getQuery("TFS INTEGRATION", myBugsQuery.id, function (err, query) {
                if (err) {
                    console.log(err);
                    console.log(query);
                }
        ...
        it('should create a query folder', function (done) {
            ...
            client.createFolder("TFS INTEGRATION", 'testFolder1', myQueries.id, function (err, folder) {
                should.not.exist(err);
                should.exist(folder);
        ...
        it('should create a query', function (done) {
            ...
            client.createQuery("TFS INTEGRATION", 'testQuery1', testFolder.id, wiql, function (err, query) {
                should.not.exist(err);
                should.exist(query);
                //console.log(query);
                query.should.have.property('id');
        ...
        it('should update a query', function (done) {
            ...
            client.updateQuery("TFS INTEGRATION", testQuery.id, 'testQuery1-updated', testFolder.id, wiql, function (err, query) {
                should.not.exist(err);
                should.exist(query);
                // console.log(query);
                query.should.have.property('id');
        ...
        it('should delete a query', function (done) {
            should.exist(testQuery);
            client.deleteQuery("TFS INTEGRATION", testQuery.id, function (err, query) {
                should.not.exist(err);
                should.exist(query);
                done();
            });
        ...
        it('should delete a query folder', function (done) {
            should.exist(testFolder);
            client.deleteFolder("TFS INTEGRATION", testFolder.id, function (err, folder) {
                should.not.exist(err);
                should.exist(folder);
                done();
            });
...
describe('VSO Client Tests Preview.2', function () {
    ...
    before(function (done) {
        client = Client.createClient(url, collection, username, password, getOptions("1.0-preview.2"));
        clientV1 = Client.createClient(url, collection, username, password, getOptions("1.0-preview.1"));
        done();
    });
    ...
    describe('Work Item Query Tests', function () {
        ...
        it('should return a list of queries', function (done) {
            // We assume an agile team project, so we will need to specifically use the
            // TFS INTEGRATION project
            client.getQueries("TFS INTEGRATION", 2, function (err, queries) {
                if (err) {
                    console.log(err);
                    console.log(queries);
                }
        ...
        it('should update a query', function (done) {
            ...
            client.updateQuery("TFS INTEGRATION", testQuery.id, 'testQuery1-updated', null, wiql, function (err, query) {
                should.not.exist(err);
                should.exist(query);
                // console.log(query);
                query.should.have.property('id');
    ...
    describe('Work Item Tests', function () {
        ...
        // Work Item Queries
        // ---------------------------------------
        it('should return a list of work items from wiql query (using V1)', function (done) {
            var wiql = 'Select [System.Id], [System.Title], [System.State] From WorkItems Where [System.WorkItemType] = \'Task\' order by [Microsoft.VSTS.Common.Priority] asc, [System.CreatedDate] desc';
            clientV1.getWorkItemIds(wiql, 'TFS Integration', function (err, ids) {
                if (err) {
                    console.log(err);
                    console.log(ids);
                }
        ...
        it('should create a work item', function (done) {
            ...
            client.createWorkItem(createWorkItemOp, "TFS INTEGRATION", "Bug", function (err, item) {
                if (err) {
                    console.log(err);
                    console.log(item);
                }
